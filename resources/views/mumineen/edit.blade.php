...
@section('title', 'Edit Mumin')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-foreground">Edit Mumin</h1>
            <p class="text-muted-foreground">Update mumin information</p>
        </div>

        <div class="bg-card p-6 rounded-lg border border-border">
            <form method="POST" action="{{ route('mumineen.update', $mumin) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="full_name" id="full_name" 
                               value="{{ old('full_name', $mumin->full_name) }}" 
                    ...
                    <select name="sabeel_code" id="sabeel_code" 
                        ...
                        @foreach($sabeels as $sabeel)
                            <option value="{{ $sabeel->sabeel_code }}" 
                                    {{ old('sabeel_code', $mumin->sabeel_code) === $sabeel->sabeel_code ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="tel" name="mobile_number" id="mobile_number" 
                           value="{{ old('mobile_number', $mumin->mobile_number) }}" 
                ...
                <div>
                    ...
                    <select name="gender" id="gender" 
                        ...
                        <option value="M" {{ old('gender', $mumin->gender) === 'M' ? 'selected' : '' }}>Male (M)</option>
                        <option value="F" {{ old('gender', $mumin->gender) === 'F' ? 'selected' : '' }}>Female (F)</option>
                    </select>
                    @error('gender')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" 
                            ...
                            <option value="resident" {{ old('type', $mumin->type) === 'resident' ? 'selected' : '' }}>Resident</option>
                            <option value="mehmaan" {{ old('type', $mumin->type) === 'mehmaan' ? 'selected' : '' }}>Mehmaan</option>
                        </select>
                        @error('type')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="misaq" id="misaq" 
                            ...
                            <option value="done" {{ old('misaq', $mumin->misaq) === 'done' ? 'selected' : '' }}>Done</option>
                            <option value="not_done" {{ old('misaq', $mumin->misaq) === 'not_done' ? 'selected' : '' }}>Not Done</option>
                        </select>
                        @error('misaq')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
