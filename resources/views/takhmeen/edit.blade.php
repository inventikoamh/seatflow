...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-foreground">Edit Takhmeen</h1>
            <p class="text-muted-foreground mt-2">Update takhmeen details</p>
        </div>
    ...
    <div class="bg-card p-6 rounded-lg border border-border">
        <form method="POST" action="{{ route('takhmeen.update', $takhmeen) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="sabeel_id" id="sabeel_id" required
                        ...
                        @foreach($sabeels as $sabeel)
                            <option value="{{ $sabeel->id }}" {{ old('sabeel_id', $takhmeen->sabeel_id) == $sabeel->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="event_id" id="event_id" required
                        ...
                        @foreach($events as $event)
                            <option value="{{ $event->id }}" {{ old('event_id', $takhmeen->event_id) == $event->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" step="0.01" min="0" required
                           value="{{ old('amount', $takhmeen->amount) }}"
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="4"
                          ...
                          class="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('notes', $takhmeen->notes) }}</textarea>
                @error('notes')
                    <p class="text-destructive text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="url" name="hof_photo" id="hof_photo"
                       value="{{ old('hof_photo', $takhmeen->hof_photo) }}"
