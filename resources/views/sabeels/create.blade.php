...
@section('title', 'Create Sabeel')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-foreground">Create New Sabeel</h1>
            <p class="text-muted-foreground">Add a new family to the system</p>
        </div>

        <div class="bg-card p-6 rounded-lg border border-border">
            <form method="POST" action="{{ route('sabeels.store') }}" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="sabeel_sector" id="sabeel_sector" 
                            ...
                            <option value="ezzi" {{ old('sabeel_sector') === 'ezzi' ? 'selected' : '' }}>Ezzi</option>
                            <option value="fakhri" {{ old('sabeel_sector') === 'fakhri' ? 'selected' : '' }}>Fakhri</option>
                            <option value="hakimi" {{ old('sabeel_sector') === 'hakimi' ? 'selected' : '' }}>Hakimi</option>
                            <option value="shujai" {{ old('sabeel_sector') === 'shujai' ? 'selected' : '' }}>Shujai</option>
                            <option value="al_masjid_us_saifee" {{ old('sabeel_sector') === 'al_masjid_us_saifee' ? 'selected' : '' }}>AL MASJID US SAIFEE</option>
                            <option value="raj_township" {{ old('sabeel_sector') === 'raj_township' ? 'selected' : '' }}>Raj Township</option>
                            <option value="zainy" {{ old('sabeel_sector') === 'zainy' ? 'selected' : '' }}>Zainy</option>
                            <option value="student" {{ old('sabeel_sector') === 'student' ? 'selected' : '' }}>Student</option>
                            <option value="mtnc" {{ old('sabeel_sector') === 'mtnc' ? 'selected' : '' }}>MTNC</option>
                            <option value="unknown" {{ old('sabeel_sector') === 'unknown' ? 'selected' : '' }}>UNKNOWN</option>
                        </select>
                        @error('sabeel_sector')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="sabeel_address" id="sabeel_address" rows="3"
                              ...
                              required>{{ old('sabeel_address') }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="sabeel_type" id="sabeel_type" 
                            ...
                            <option value="regular" {{ old('sabeel_type') === 'regular' ? 'selected' : '' }}>Regular</option>
                            <option value="student" {{ old('sabeel_type') === 'student' ? 'selected' : '' }}>Student</option>
                            <option value="res_without_sabeel" {{ old('sabeel_type') === 'res_without_sabeel' ? 'selected' : '' }}>Resident Without Sabeel</option>
                            <option value="moallemeen" {{ old('sabeel_type') === 'moallemeen' ? 'selected' : '' }}>Moallemeen</option>
                            <option value="regular_lock_joint" {{ old('sabeel_type') === 'regular_lock_joint' ? 'selected' : '' }}>Regular Lock/Joint</option>
                            <option value="left_sabeel" {{ old('sabeel_type') === 'left_sabeel' ? 'selected' : '' }}>Left Sabeel</option>
                        </select>
                        @error('sabeel_type')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
