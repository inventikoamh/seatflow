...
@section('content')
    ...
    <div class="mb-8 animate-fade-in">
        <div class="bg-card rounded-lg border border-border p-6 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-foreground mb-2">Create New User</h1>
                    <p class="text-muted-foreground">Add a new user to the system and assign roles</p>
                </div>
    ...
    <div class="bg-card rounded-lg border border-border shadow-sm">
        <div class="p-6 border-b border-border">
            ...
            <p class="text-muted-foreground text-sm mt-1">Enter the user's personal information and assign roles.</p>
        </div>
        
        <form method="POST" action="{{ route('users.store') }}" class="p-6" data-validate>
            @csrf
            ...
            <div class="mt-6">
                ...
                <input id="password" type="password" name="password" required class="w-full px-3 py-2 border border-input rounded-md shadow-sm placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-ring bg-background text-foreground @error('password') border-destructive @enderror" placeholder="Enter password">
                @error('password')
                    <p class="mt-1 text-sm text-destructive">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                <input id="password_confirmation" type="password" name="password_confirmation" required class="w-full px-3 py-2 border border-input rounded-md shadow-sm placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-ring bg-background text-foreground @error('password_confirmation') border-destructive @enderror" placeholder="Confirm password">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    @foreach($roles as $role)
                        <label class="flex items-center space-x-3 p-3 border border-input rounded-md hover:bg-accent/50 cursor-pointer transition-colors">
                            <input type="checkbox" name="roles[]" value="{{ $role->id }}" {{ in_array($role->id, old('roles', [])) ? 'checked' : '' }} class="rounded border-input text-primary focus:ring-ring">
                            <div class="flex-1">
